...
def upload_blob_file(blob_service_client, container_name, filepathstring, filenamestring):
    container_client = blob_service_client.get_container_client(container=container_name) with open(file=os.path.join(filepathstring, filenamestring), mode="rb") as data:
        blob_client = container_client.upload_blob(name=filenamestring, data=data, overwrite=True)

upload_blob_file(blob_service_client, 'storagepractice123', './', 'samplefile.jpg')
